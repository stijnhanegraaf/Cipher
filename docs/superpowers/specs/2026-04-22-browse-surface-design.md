...
- **Syntax highlighting** — replace the plain `<code>` rendering with `rehype-shiki` using one light + one dark theme picked by the app theme. Load grammars lazily per language on first use and cache in a module-scope `Map`. Seeded grammar list covers the common set: ts, tsx, js, jsx, py, go, rust, swift, java, kotlin, rb, php, sql, sh, bash, zsh, yaml, json, toml, html, css, md. Languages outside this set fall back to a plain monospace block rather than bundling every grammar.
